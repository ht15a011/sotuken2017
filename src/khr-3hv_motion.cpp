...
int main(int argc, char** argv) {
  ...
  tsm.header.frame.id = "base_link";          // 全ての変換はこのベースフレームを参照して行われる。
  tsm.child_frame_id = "base_link";             // ロボットのベースとなるリンクを設定

  // 以下ではロボットのジョイント数をnとしている。
  // ジョイント用メッセージの初期化
  jsm.name.resize(20);                                // ジョイント数によりメッセージサイズを設定
  tsm.position.resize(20);
